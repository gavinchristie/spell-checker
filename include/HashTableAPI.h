 ...
 * Modified by Gavin Christie, November 2017, to improve abstraction
 * 	- Added removeDataInMap and lookupDataInMap functions to wrap removeData and lookupData
 *	- Added convertToKey to HTable struct to all user to create the key
 */
...
struct HTable
{
    ...
    int (*convertToKey)(void *data); ///< function pointer to a function that converts the data into a key that can be used in hashing
...
/** User call this function to remove data from the hash table. Function is used to generate key from data
 * and call the removeData function with that key
 * @pre Hash table must exist and has memory allocated to it
 * @param hashTable The hash table containing the data being removed
 * @param data Pointer to the data being looked for
 **/
...
/** Wrapper function used to generate key before calling the lookupData to increase abstraction
 * @pre Hash table must exist and have allocated memory
 * @param hashTable Pointer to the hash table contain information
 * @param data Pointer to the data being looked up
 * @return Returns a pointer to the data in the hash table, returns NULL if no match is found
 **/
void *lookupDataInMap(HTable *hashTable, void *data);

/** Function to print off the contents of a hash table with the following format
 * <index>:<key>:<data>. Uses the user defined printNode to print the data.
 * @param hashTable Pointer to the hash table being printed
 **/
